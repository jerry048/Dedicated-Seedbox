...
Options:
  --storage auto|hdd|ssd|sata-ssd|nvme   Storage class for dirty page and scheduler policy. Default: auto
...
Policy:
  ...
  - Disk scheduler: HDD mq-deadline, SATA SSD kyber, NVMe none, best-effort
USAGE
}
...
tuning::storage_class() {
  local requested dev rotational
  ...
  dev="$(tuning::root_block_device || true)"
  if [[ "${dev}" == nvme* ]]; then
    printf 'nvme\n'
    return 0
  fi
...
tuning::write_sysctl() {
  local out iface mem link storage
  ...
  storage="$(tuning::storage_class)"
  {
    ...
    printf '# detected_mem_mib=%s detected_link_mbps=%s storage=%s interface=%s\n' "${mem}" "${link}" "${storage}" "${iface:-auto}"
...
tuning::write_defaults() {
  local iface link_iface link storage txq initcwnd enable_ring enable_rss enable_rps enable_rfs enable_xps enable_initial_cwnd enable_disk_scheduler
...
seedbox_tuning_known_key() {
  case "$1" in
    SEEDBOX_TUNING_IFACE|SEEDBOX_TUNING_LINK_MBPS|SEEDBOX_TUNING_STORAGE|SEEDBOX_TXQUEUELEN|SEEDBOX_INITIAL_CWND|SEEDBOX_ENABLE_RING|SEEDBOX_ENABLE_RSS|SEEDBOX_ENABLE_RPS|SEEDBOX_ENABLE_RFS|SEEDBOX_ENABLE_XPS|SEEDBOX_ENABLE_INITIAL_CWND|SEEDBOX_ENABLE_DISK_SCHEDULER) return 0 ;;
    *) return 1 ;;
  esac
}
...
block_class() {
  ...
  if [[ -r "/sys/block/${dev}/queue/rotational" && "$(cat "/sys/block/${dev}/queue/rotational")" == "1" ]]; then
    printf 'hdd\n'
  else
    printf 'sata-ssd\n'
  fi
...
set_disk_scheduler() {
  local scheduler_file dev klass available candidate
  for scheduler_file in /sys/block/*/queue/scheduler; do
    [[ -w "${scheduler_file}" ]] || continue
    dev="$(basename -- "$(dirname -- "$(dirname -- "${scheduler_file}")")")"
    case "${dev}" in loop*|ram*|sr*|fd*|dm-*|md*) continue ;; esac
    klass="$(block_class "${dev}")"
    available="$(cat "${scheduler_file}" 2>/dev/null || true)"
    case "${klass}" in
      hdd) for candidate in mq-deadline deadline; do [[ "${available}" == *"${candidate}"* ]] && { printf '%s' "${candidate}" >"${scheduler_file}" 2>/dev/null || true; log "${dev}: scheduler=${candidate}"; break; }; done ;;
      nvme) [[ "${available}" == *none* ]] && { printf 'none' >"${scheduler_file}" 2>/dev/null || true; log "${dev}: scheduler=none"; } ;;
      *) for candidate in kyber mq-deadline none; do [[ "${available}" == *"${candidate}"* ]] && { printf '%s' "${candidate}" >"${scheduler_file}" 2>/dev/null || true; log "${dev}: scheduler=${candidate}"; break; }; done ;;
    esac
  done
}
